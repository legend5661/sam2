...
def main():
    ...
    if args.pretrain:
        # 目前只需要加载预训练模型的 Image encoder 部分的权重

        print(args.pretrain)
        # 加载预训练模型的权重
        pretrained_weights = torch.load(args.pretrain)
        pretrained_weights = pretrained_weights['model']
        
        # 获取模型的当前状态字典
        model_dict = net.state_dict()
        
        # 过滤出预训练模型中 Image encoder 部分的权重
        pretrained_image_encoder_weights = {k: v for k, v in pretrained_weights.items() if k.startswith('image_encoder')}
        
        # 更新模型的状态字典
        model_dict.update(pretrained_image_encoder_weights)
