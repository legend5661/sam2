...
class PromptEncoder3D(nn.Module):
    def __init__(
        ...
        self.pe_layer = PositionEmbeddingRandom3D(embed_dim)
    ...
    def _embed_points(
        ...
        print('self.not_a_point_embed.weight shape:', self.not_a_point_embed.weight.shape)
...
class PositionEmbeddingRandom3D(nn.Module):
    """
    Positional encoding using random spatial frequencies with learnable linear projection.
    """

    def __init__(self, embed_dim: int, scale: Optional[float] = None) -> None:
        super().__init__()
        if scale is None or scale <= 0.0:
            scale = 1.0

        self.num_pos_feats = embed_dim // 3  # 使用 embed_dim // 3 作为基础特征数量
        self.embed_dim = embed_dim

        # 初始化随机高斯矩阵，用于位置编码
        self.register_buffer(
            "positional_encoding_gaussian_matrix",
            scale * torch.randn((3, self.num_pos_feats)),
        )

        # 添加可学习的线性投影层，将输出映射到 embed_dim
        self.linear_projection = nn.Linear(3 * self.num_pos_feats, self.embed_dim)

    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        """Positionally encode points that are normalized to [0,1]."""
        coords = 2 * coords - 1  # 将坐标映射到 [-1, 1] 范围
        coords = coords @ self.positional_encoding_gaussian_matrix
        coords = 2 * np.pi * coords

        # 使用 sin 和 cos 编码
        encoded = torch.cat([torch.sin(coords), torch.cos(coords), torch.sin(coords)], dim=-1)

        # 使用可学习的线性投影层映射到 embed_dim
        projected_encoding = self.linear_projection(encoded)

        return projected_encoding
    ...
    def forward(self, size: Tuple[int, int, int]) -> torch.Tensor:
        ...
        return pe.permute(3, 0, 1, 2)  # 返回维度为 C x X x Y x Z
    ...
    def forward_with_coords(
        ...
        # 使用线性投影后的编码返回
        return self._pe_encoding(coords.to(torch.float))  # B x N x embed_dim
