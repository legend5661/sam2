...
class Block3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 这里的x是一个windows列表，每个元素是一个window，x的形状为（B * num_windows, window_size, window_size, window_size, C）
        x = self.attn(x)
...
def window_partition3D(x: torch.Tensor, window_size: int) -> Tuple[torch.Tensor, Tuple[int, int, int]]:
    ...
    # （B, D, H, W, C）->(B, Dp//window_size, window_size, Hp//window_size, window_size, Wp//window_size, window_size, C),Dp//window_size深度维度上分块的个数
    x = x.view(B, Dp // window_size, window_size, Hp // window_size, window_size, Wp // window_size, window_size, C)
    # 变成了（B * (Dp // window_size) * (Hp // window_size) * (Wp // window_size)，window_size, window_size, window_size, C），第一维就是batch_size * num_windows
