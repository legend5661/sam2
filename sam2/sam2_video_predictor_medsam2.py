...
class SAM2VideoPredictor(SAM2_3D):
    ...
    def init_state(
        ...
        images, video_depth, video_height, video_width = load_video_frames(
        ...
        inference_state["video_depth"] = video_depth
    ...
    def val_init_state(
        ...
        """Initialize a inference state."""
        if video_height is None or video_width is None or video_depth is None:
    ...
    def add_new_points(
        ...
        if normalize_coords:
            video_D = inference_state["video_depth"]
            video_H = inference_state["video_height"]
            video_W = inference_state["video_width"]
            points = points / torch.tensor([video_D, video_H, video_W]).to(points.device)
    ...
    def add_new_bbox(
        ...
        bbox_coords = bbox.reshape(-1, 3, 2)
        bbox_labels = torch.tensor([2, 3, 4], dtype=torch.int)
    ...
    def train_add_new_points(
        ...
        """Add new points to a 3D frame.在某个指定的帧中添加某个指定的对象的点击prompt"""
        obj_idx = self._obj_id_to_idx(inference_state, obj_id)
        point_inputs_per_frame = inference_state["point_inputs_per_obj"][obj_idx]
        mask_inputs_per_frame = inference_state["mask_inputs_per_obj"][obj_idx]
    ...
    def add_new_mask(
        ...
        assert mask.dim() == 3
        mask_D, mask_H, mask_W = mask.shape
        mask_inputs_orig = mask[None, None]  # add batch and channel dimension
        mask_inputs_orig = mask_inputs_orig.float().to(inference_state["device"])

        # resize the mask if it doesn't match the model's image size
        if mask_D != self.image_size or mask_H != self.image_size or mask_W != self.image_size:
    ...
    def _get_orig_video_res_output(self, inference_state, any_res_masks):
        ...
        if any_res_masks.shape[-3:] == (video_D, video_H, video_W):
            video_res_masks = any_res_masks
        else:
            video_res_masks = torch.nn.functional.interpolate(
                any_res_masks,
                size=(video_D, video_H, video_W),
                mode="trilinear",
                align_corners=False,
            )
    ...
    def _consolidate_temp_output_across_obj(
        ...
        if consolidate_at_video_res:
            ...
            consolidated_D = inference_state["video_depth"]
            consolidated_H = inference_state["video_height"]
            consolidated_W = inference_state["video_width"]
            consolidated_mask_key = "pred_masks_video_res"
        else:
            consolidated_D = consolidated_H = consolidated_W = self.image_size // 4
        ...
        consolidated_out = {
            ...
            consolidated_mask_key: torch.full(
                size=(batch_size, 1, consolidated_D, consolidated_H, consolidated_W),
        ...
        for obj_idx in range(batch_size):
            ...
            if obj_mask.shape[-3:] == consolidated_pred_masks.shape[-3:]:
                consolidated_pred_masks[obj_idx : obj_idx + 1] = obj_mask
            else:
                # Resize first if temporary object mask has a different resolution
                resized_obj_mask = torch.nn.functional.interpolate(
                    obj_mask,
                    size=consolidated_pred_masks.shape[-3:],
                    mode="trilinear",
                    align_corners=False,
                )
