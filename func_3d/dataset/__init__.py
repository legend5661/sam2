...
from .mydataset import MyDataSet_3D
...
def get_dataloader(args):
    ...
    if args.dataset == 'btcv':
        ...
        '''mydataset data'''
        mydataset_train_dataset = MyDataSet_3D(args, args.data_path, transform = None, transform_msk= None, mode = 'Training', prompt=args.prompt)
        mydataset_test_dataset = MyDataSet_3D(args, args.data_path, transform = None, transform_msk= None, mode = 'Test', prompt=args.prompt)

        nice_train_loader = DataLoader(mydataset_train_dataset, batch_size=1, shuffle=True, num_workers=8, pin_memory=True)
        nice_test_loader = DataLoader(mydataset_test_dataset, batch_size=1, shuffle=False, num_workers=1, pin_memory=True)
        '''end'''
