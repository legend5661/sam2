...
def random_click_3d(mask, point_labels=1, seed=None):
    """
    在3D的mask中生成随机点击点
    """
    # 检查是否所有的mask都是黑色的
    ...
    if seed is not None:
        ...
        rand_num = random.randint(0, len(indices) - 1)
    output_index_2 = indices[rand_num][0]
    output_index_1 = indices[rand_num][1]
    output_index_0 = indices[rand_num][2]
    return point_labels, np.array([output_index_0, output_index_1, output_index_2])
...
def generate_bbox_3d(mask, variation=0, seed=None):
    if seed is not None:
        np.random.seed(seed)
    # check if all masks are black
    if len(mask.shape) != 3:
        current_shape = mask.shape
        raise ValueError(f"Mask shape is not 3D, but {current_shape}")
    max_label = max(set(mask.flatten()))
    if max_label == 0:
    ...
    mid_z = (z0 + z1) / 2
    if variation > 0:
        num_rand = np.random.randn(3) * variation
        w *= 1 + num_rand[0]
        h *= 1 + num_rand[1]
        d *= 1 + num_rand[2]
        x1 = mid_x + w / 2
